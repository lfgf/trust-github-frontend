...
@Component({
  ...
  template: `
    <!-- The Card -->
    <div (click)="openModal()" class="group cursor-pointer relative p-4 sm:p-6 rounded-2xl bg-kojinx-panel border border-kojinx-border hover:border-white/30 transition-colors">
      <div class="flex items-center justify-between gap-3.5">
        <div class="flex items-center gap-3.5 min-w-0">
          <div class="w-10 h-10 sm:w-12 sm:h-12 rounded-xl bg-white/10 flex items-center justify-center border border-white/20 shrink-0 aspect-square">
            <svg class="w-5 h-5 sm:w-6 sm:h-6 text-white shrink-0" fill="currentColor" viewBox="0 0 24 24" aria-hidden="true">
          ...
          <div class="min-w-0">
            <p class="text-sm sm:text-base font-semibold text-white group-hover:text-gray-300 transition-colors truncate">GitHub</p>
            <p class="text-xs sm:text-sm text-kojinx-text-muted mt-0.5 truncate">
              {{ isConnected ? 'Discover and clone repositories' : 'Link account to import repositories' }}
            </p>
          </div>
        </div>
        <div class="shrink-0">
          @if (isConnected) {
            <span class="inline-flex items-center gap-1.5 px-3 py-1.5 sm:px-4 sm:py-2 rounded-xl bg-green-500/10 border border-green-500/30 text-green-400 text-xs sm:text-sm font-bold font-mono">
            ...
            <span class="inline-flex items-center gap-1.5 px-3.5 py-1.5 sm:px-4 sm:py-2 rounded-xl bg-white/5 border border-white/10 text-white text-xs sm:text-sm font-bold font-mono group-hover:bg-white/10 transition-colors">
              Connect
            </span>
          }
        </div>
